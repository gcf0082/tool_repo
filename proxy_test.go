...
func newUpstream(t *testing.T) *httptest.Server {
	t.Helper()
	root := t.TempDir()
	writeFile(t, root, "fzf/0.1.0/linux-amd64/fzf.tar.gz", "fzf-bin-from-upstream")
...
func TestProxyGetToolPassesThrough(t *testing.T) {
	...
	if cd := resp.Header.Get("Content-Disposition"); !strings.Contains(cd, `filename="fzf.tar.gz"`) {
